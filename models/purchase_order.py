...
class PurchaseOrder(models.Model):
    ...
    # Área solicitante
    requesting_area = fields.Selection([
        ...
        ('calidad', 'CALIDAD'),
        ('otros', 'OTROS'),
    ], string='Área Solicitante', default='produccion')
    ...
    def get_supplier_bank_info(self):
        """Retorna información bancaria del proveedor"""
        if self.partner_id and self.partner_id.bank_ids:
            # Tomar la primera cuenta bancaria
            bank_account = self.partner_id.bank_ids[0]
            return {
                'bank_name': bank_account.bank_id.name if bank_account.bank_id else '',
                'account_number': bank_account.acc_number or '',
                'cci_number': bank_account.l10n_pe_cci or '',
